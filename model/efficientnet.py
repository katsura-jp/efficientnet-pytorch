...
class EfficientNet(nn.Module):
    def __init__(self, num_classes=1000, width_coef=1., depth_coef=1., scale=1.,dropout_ratio=0.2, se_ratio=0.25):
        ...
        self.stage2 = MBConv(channels[0], channels[1], repeats[0], kernel_size=3, stride=strides[0], expand=expands[0], se_ratio=se_ratio)
        self.stage3 = MBConv(channels[1], channels[2], repeats[1], kernel_size=3, stride=strides[1], expand=expands[1], se_ratio=se_ratio)
        self.stage4 = MBConv(channels[2], channels[3], repeats[2], kernel_size=5, stride=strides[2], expand=expands[2], se_ratio=se_ratio)
        self.stage5 = MBConv(channels[3], channels[4], repeats[3], kernel_size=3, stride=strides[3], expand=expands[3], se_ratio=se_ratio)
        self.stage6 = MBConv(channels[4], channels[5], repeats[4], kernel_size=5, stride=strides[4], expand=expands[4], se_ratio=se_ratio)
        self.stage7 = MBConv(channels[5], channels[6], repeats[5], kernel_size=5, stride=strides[5], expand=expands[5], se_ratio=se_ratio)
        self.stage8 = MBConv(channels[6], channels[7], repeats[6], kernel_size=3, stride=strides[6], expand=expands[6], se_ratio=se_ratio)
